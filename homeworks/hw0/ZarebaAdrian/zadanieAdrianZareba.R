...
loginnew <- employees[(employees$surname == "Insecure") & (employees$name == "John"),]$login
proton(action = "login", login=loginnew)

top1000passwords
proton(action = "login", login=loginnew, password="ABC")

for(i in top1000passwords)
{
  proton(action = "login", login=loginnew, password=i)
}
